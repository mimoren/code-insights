...
```
Phase 1: Plan    → classify the request, create a task only for complex work, then write planning artifacts
...
<!-- CLIM managed: clim:no-task-policy:v2 -->
No active task. Classify the current turn without asking for task-creation consent.
Read-only questions, analysis, research, and small local changes may be handled directly without a Trellis task.
For complex, multi-step, cross-module, or long-lived work, create a Trellis task and enter planning automatically.
[/workflow-state:no_task]
...
Goal: classify the request, create a Trellis task when the work requires durable planning, and produce the planning artifacts required before implementation.

#### 1.0 Create task `[required · when needed]`

Create a task for complex, multi-step, cross-module, or long-lived work. Read-only questions, research, and small local changes do not require a task. When a task is needed, create it directly; task creation consent is not a separate per-turn gate. The command sets status to `planning`, writes `task.json`, creates a default `prd.md`, and auto-targets the new task when session identity is available:
...
- No active task must classify the request first; read-only and small work may proceed without a task, while complex work creates one and enters planning automatically.
