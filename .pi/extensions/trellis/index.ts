...
export function readPiDispatchMode(repoRoot: string): "inline" | "sub-agent" {
...
export function buildContext(root: string, agent: string, key: string | null): string {
  const dir = readTaskDir(root, key);
  if (!dir)
...
export default function trellisExtension(pi: {
  ...
  pi.on?.("before_agent_start", (event, ctx) => {
    ...
    if (previousRuntimeContext === undefined) {
      // Startup already includes the initial overview; inject only the workflow breadcrumb.
